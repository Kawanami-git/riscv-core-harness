...
ee_s16 calc_func(ee_s16* pdata, core_results* res)
{
  ...
  {                                     /* otherwise calculate and cache the result */
    ...
    case 0:
      ...
      retval =
          core_bench_state(res->size, res->memblock[3], res->seed1, res->seed2, dtype, res->crc);
      if (res->crcstate == 0)
      {
        res->crcstate = retval;
      }
...
list_head* core_list_init(ee_u32 blksize, list_head* memblock, ee_s16 seed)
{
  ...
  ee_u32 size = (blksize / per_item) - 2; /* to accommodate systems with 64b pointers, and make sure
  ...
  while (finder->next != NULL)
  {
    ...
    else
    {
      ...
      finder->info->idx = 0x3fff & (((i & 0x07) << 8) | pat); /* make sure the mixed items end up
