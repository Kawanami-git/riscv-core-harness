...
/*
 * riscv-core-harness
 * Custom implementation to measure time through mcycle
 * counter. The Coremark value returned is now
 * the total number of clock cycles necessary to realize
 * the specified number of iterations.
 */
#define CORETIMETYPE         uint32_t
#define GETMYTIME(_t)        (*_t = csr_read(CSR_MCYCLE))
#define MYTIMEDIFF(fin, ini) ((fin) - (ini))
/**/
...
ee_u8 core_stop_parallel(core_results* res)
{
  ...
  int recsize =
      recvfrom(res->port.sock, &(res->crc), 8, 0, (struct sockaddr*)&(res->port.sa), &fromlen);
  if (recsize < 0)
  {
