...
main(int argc, char* argv[])
{
  ...
  /*
   * riscv-core-harness
   * Iteration is now provided at build time and not through arguments.
   */
...
#error "Please define a way to initialize a memory block."
  ...
  /*
   * riscv-core-harness
   * Change the 'iteration' value computation.
   * Iteration value is provided at build time and not through arguments.
   */
  /* automatically determine number of iterations if not set */
  if (results[0].iterations == 0) // modified
  {
  ...
  case 0x8a02: /* seed1=0, seed2=0, seed3=0x66, size 2000 per algorithm */
    known_id = 0;
    ee_printf("6k performance run parameters for CycleMark.\n");
    break;
  case 0x7b05: /*  seed1=0x3415, seed2=0x3415, seed3=0x66, size 2000 per
                  algorithm */
    known_id = 1;
    ee_printf("6k validation run parameters for CycleMark.\n");
    break;
  case 0x4eaf: /* seed1=0x8, seed2=0x8, seed3=0x8, size 400 per algorithm
                */
    known_id = 2;
    ee_printf("Profile generation run parameters for CycleMark.\n");
    break;
  case 0xe9f5: /* seed1=0, seed2=0, seed3=0x66, size 666 per algorithm */
    known_id = 3;
    ee_printf("2K performance run parameters for CycleMark.\n");
    break;
  case 0x18f2: /*  seed1=0x3415, seed2=0x3415, seed3=0x66, size 666 per
                  algorithm */
    known_id = 4;
    ee_printf("2K validation run parameters for CycleMark.\n");
    break;
  default:
    total_errors = -1;
    break;
  ...
  /*
   * riscv-core-harness
   * Remove the limitation of minimum duration.
   */
  ...
  if (total_errors == 0)
  {
    ee_printf("Correct operation validated. See README.md for run and reporting "
              "rules.\n");
#if HAS_FLOAT
    if (known_id == 3)
    {
      ee_printf("CycleMark 1.0 : %f / %s %s",
                default_num_contexts * results[0].iterations / time_in_secs(total_time),
                COMPILER_VERSION,
                COMPILER_FLAGS);
